...
MemberJunction ships a CSS custom property (design token) system; bizapps-forms consumes it via `@memberjunction/ng-ui-components` rather than defining its own. The respondent widget layers its own `--mjf-*` tokens on top, fed from `FormStyle.Tokens` so a form can be themed per-distribution without code. **Every color in component CSS MUST use design tokens.** Hardcoded hex values (`#264FAF`, `#333`, `#f5f5f5`, etc.) break dark mode, prevent white-labeling, and create maintenance debt.
...
## In bizapps-forms specifically

- All 7 component stylesheets are plain `.css` — there is no Sass in this repo, so ignore any
  `.scss`-only guidance above.
- **`npm run lint:ui` is the enforcing gate** (`scripts/check-ui-tokens.mjs`). It fails the build on a
  hardcoded colour. Run it before pushing; CI runs it too.
- The gate also contains a `mj-btn` check that is **deliberately disabled** — this repo uses raw
  `<button>` by convention and has adopted no `mj-btn` class. It reports that state on every run rather
  than staying silent, so nobody mistakes "disabled" for "passing".
- The respondent widget is a **custom element with its own shadow root**. Host-page CSS does not cascade
  into it, so tokens must be set on the element itself — this is why `FormStyle` tokens are injected as
  inline custom properties rather than relying on inheritance.
