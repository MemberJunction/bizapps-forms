...
Pinning model (verified against this tree, do not violate). **Two groups that move on different schedules — this is the single most important thing on this page, and PR #233 changed it:**

- **Host-tracking pins** — which MJ build you develop and run against. `apps/*` use **exact** `X.Y.Z` in `dependencies`; root `@memberjunction/cli` and `packages/Server`'s `@memberjunction/generic-database-provider` `devDependencies` are **caret** (they were exact until #233; an exact pin on a package that can be a workspace sibling defeats linking rather than selecting, so pnpm fetches a published copy and a second `@memberjunction/core` lands beside the linked one, giving `packages/Server` 9 × TS2322 on `UserInfo`'s private fields). Also the root `pnpm.overrides` block, which is **exact and actually binds** — see below.
- **The supported-host declaration** — which MJ hosts this app claims to work on. `packages/*` carry MJ only as `peerDependencies`, and since #233 those are anchored at a **prerelease**: `^X.Y.Z-edge.N`, not `^X.Y.Z`. `mj-app.json`'s `mjVersionRange` is `>=X.Y.Z-edge.N <(major+1).0.0` and is **derived** from `packages/Entities`' own MJ pin by `scripts/sync-app-version.mjs`; `build-and-test` checks that derivation, so it never moves independently.

**Why the peers carry a prerelease at all.** Semver admits a prerelease only when a comparator shares its exact `major.minor.patch` *and* itself carries a prerelease tag, so `^6.1.1` admits **no** `-edge.N` host whatsoever. A `6.1.0-edge.6` host then fails ERESOLVE, which `mj app install` reports as "npm install failed — log in to npm" before finalizing the app **Disabled** — that is #211. `npm run lint:mj-ranges` (a step in the required `build-and-test` check) now refuses any MJ peer that admits no prerelease of its own tuple, or whose floor does not match `mjVersionRange`'s floor exactly.

**So a host patch upgrade does not touch the peers.** `^6.1.0-edge.6` already admits 6.1.1, 6.1.2 and every later 6.x — verify with `semver.satisfies('6.1.2','^6.1.0-edge.6')`. Flattening it to `^6.1.2` for a host bump re-breaks every Edge host and turns CI red. `bump-pins.sh` enforces this split for you: a target with **no** prerelease moves only the host-tracking pins and says so; a target **with** one (`6.2.0-edge.0`) moves the peers and `mjVersionRange` too. Always use it, never hand-edit pins — and it now runs `lint:mj-ranges` against its own output before reporting success. Angular is pinned separately from MJ: `@angular/*` peers in `packages/*` stay caret at the platform pin (`^21.2.22` since the 6.1.0 upgrade) with exact anchors in consuming packages' `devDependencies`. **Three version lines, not one** — `@angular/cdk` is `21.2.14`, the build tooling (`@angular/cli`, `@angular/build`, `@angular-devkit/build-angular`) is `21.2.23`, everything else `21.2.22`; collapsing them into one number is what #211 was. **And the pin that actually binds is `pnpm.overrides` in the root `package.json`, not the anchor** — an override outranks every manifest, so on the 6.1.0 upgrade bumping only the anchor left the regenerated lockfile still on `@angular/core@21.1.3` while `@angular/cdk` moved, purely because CDK is the one package the override list omits. Bump overrides and anchors together and confirm with `grep -c '21\.1\.3' pnpm-lock.yaml` returning 0. An MJ pin bump does not *automatically* touch Angular — but check MJ's own platform version every time, because 6.1.0 moved it and left this repo's peers unsatisfiable.
...
3. **Bump pins:** `bash .claude/skills/mj-upgrade/scripts/bump-pins.sh <version>` — it prints a verification summary and exits non-zero on stragglers. The count depends on which group the target moves (see the pinning model above):
   - a **release** target (`6.1.2`) → **14 pins across 2 files** (root + `apps/MJAPI`), and it lists the peer manifests it deliberately left alone;
   - a **prerelease** target (`6.2.0-edge.0`) → **55 pins across 7 files**, peers and `mjVersionRange` included.

   (2026-09-14: it was 70/8 before `apps/MJExplorer` was reduced to stale build leftovers with no `package.json` — the script prints `skip (missing)` for it, which is correct, not a warning.) The script finishes by running `lint:mj-ranges` on what it just wrote; if that refuses the tree, do not commit it.
