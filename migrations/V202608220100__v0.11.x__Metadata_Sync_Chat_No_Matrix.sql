-- ---------------------------------------------------------------------------------------------
-- Metadata seed delta: stop offering a question type the chat cannot build.
--
-- WHAT THIS CARRIES
--   * 'MJ_BizApps_Forms: Chat Assistant' template text only.
--
-- WHY. `Matrix` was in the list of types the prompt tells the model it may use, and the model duly
-- proposed one. It cannot be built from here: a matrix splits its choices across ROWS and COLUMNS,
-- and this vocabulary's "options" is a single flat list with no way to say which axis a choice
-- belongs to. Every option therefore lands as a row, the grid gets no columns, and the widget
-- renders a table with nothing to click — a question nobody can answer, and nobody can submit past
-- if it is required. `form-blueprint.ts` documents that failure at the `matrixAxis` field the
-- BLUEPRINT route added to avoid it; the edit vocabulary has no equivalent.
--
-- The code refuses it either way (`TYPES_THE_CHAT_CANNOT_BUILD`, both as a destination and as a
-- source). This is the other half: without it the model keeps proposing a Matrix, and every such
-- turn costs a round trip to produce a refusal. Named in "WHAT YOU STILL CANNOT DO" as well, with
-- the control that CAN do it — the question palette down the left of the builder, which is where
-- a type is chosen, since the question panel shows the type as a read-only pill.
-- ---------------------------------------------------------------------------------------------
...
a grid, use `unsupported` and send them to the question palette down the left of the builder.
