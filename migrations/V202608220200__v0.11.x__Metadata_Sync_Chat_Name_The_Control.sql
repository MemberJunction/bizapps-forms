-- ---------------------------------------------------------------------------------------------
-- Metadata seed delta: the prompt was naming a control that does not exist.
--
-- WHAT THIS CARRIES
--   * 'MJ_BizApps_Forms: Chat Assistant' template text only.
--
-- WHY, and this one was found by smoke testing rather than by reading. Asked for a grid question,
-- the assistant correctly declined and then said: "You can add one using the + button in the left
-- panel to select a Matrix question." There is no + button for questions. The two + buttons on the
-- builder canvas add a SECTION and an ENDING, so an author following that ends up with an empty
-- page; questions are added by clicking a TYPE TILE in the palette down the left.
--
-- The model was not inventing it. This template's own "name the control" rule said, verbatim, "the
-- + button in the left panel for a new question" — and in the same sentence claimed the question's
-- panel on the right edits its TYPE, which it does not: the type is a plain label there, and
-- changing one means removing the question and adding a new one. The model followed the prompt
-- faithfully and the prompt was wrong, which is also where the same wording in two of the code's
-- own refusal messages came from.
--
-- Now checked against the templates rather than remembered: the palette tiles add questions, the
-- right-hand panel edits wording, options (including a matrix's rows and columns) and required-ness
-- but never type, and the Design tab owns colour, size, font and radius.
-- ---------------------------------------------------------------------------------------------
