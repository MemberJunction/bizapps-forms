...
const MUTANTS = [
    ...
    ['schemasync/qualified-callee', 'the accounting counts only a QUALIFIED callee, so a procedure named in prose is not read as a call',
        '    for (const call of sql.matchAll(/[.["](sp\\w+)/gi)) {', '    for (const call of sql.matchAll(/\\b(sp\\w+)/gi)) {'],
    ['schemasync/unbracketed-call', 'a call counts whether or not the procedure name is bracketed — T-SQL makes that optional',
        '    for (const call of sql.matchAll(/[.["](sp\\w+)/gi)) {', '    for (const call of sql.matchAll(/[["](sp\\w+)/gi)) {'],
    ['schemasync/unqualified-exec', 'an `EXEC spX` with no schema qualifier is counted too',
        '    for (const call of sql.matchAll(/\\bEXEC(?:UTE)?\\s+(sp\\w+)/gi)) {', '    for (const call of []) {'],
    ['schemasync/repeatables-gated', 'R__ repeatables are gated, not skipped for lacking a version stamp',
        "            const version = stamp === null ? (/^R__/.test(file) ? '999999999999' : null) : stamp[1];\n            if (version === null || version < SCHEMA_SYNC_GATE_FROM) continue;",
        '            if (stamp === null || stamp[1] < SCHEMA_SYNC_GATE_FROM) continue;\n            const version = stamp[1];'],
