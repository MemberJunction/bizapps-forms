...
export const MUTANTS = [
  // --- public-submit: the ONE ownership rule on the anonymous write path --------------------
  {
    ...
    find: "  if (owner === '' || owner === foldSessionId(caller.sessionId)) {",
    replace: "  if (true || owner === '' || owner === foldSessionId(caller.sessionId)) {",
  ...
  },
  {
    ...
    file: 'packages/Server/src/public-submit/persistence.service.ts',
    find: "  if (owner === '' || owner === foldSessionId(caller.sessionId)) {",
    replace: "  if (owner === '' || foldSessionId(caller.sessionId) === '' || owner === foldSessionId(caller.sessionId)) {",
    suite: 'packages/Server',
  },
  {
    ...
    file: 'packages/Server/src/public-submit/persistence.service.ts',
    find: "  return scope !== '' && scope === foldSessionId(response.ID);",
    replace: "  return scope === foldSessionId(response.ID);",
    suite: 'packages/Server',
  },
