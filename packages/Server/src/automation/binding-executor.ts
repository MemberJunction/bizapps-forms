...
export interface BindingTargetGateway {
  ...
  describeEntity(entityName: string): Promise<ReadonlySet<string> | null>;
...
export async function executeBinding(input: ExecuteBindingInput): Promise<BindingResult> {
  ...
  try {
    writableFields = await gateway.describeEntity(config.targetEntityName);
  } catch (error) {
  ...
  const identityFields = (config.identityRule.match ?? []).map((m) => m.targetField);
  ...
  if (plan.size === 0 && match.existing) {
    // Nothing changed. Saving anyway would stamp __mj_UpdatedAt and fabricate a record-change row,
    // turning every replay into an apparent edit.
    return { ok: true, outcome: { kind: 'Unchanged', targetRecordId: match.existing.recordId, writtenFields: [] } };
  }
