...
/**
 * The alert text for a failed load or export: what was being done, then why it failed.
 *
 * The dashboard used to show the thrown message INSTEAD of the action whenever a real Error
 * arrived, so the context sentence was discarded for every real failure and the reader was
 * left with a transport string ("GraphQL Error (Code: unknown)") that says nothing about what
 * failed. The action is always kept; the error's message is appended only when it has one.
 */
