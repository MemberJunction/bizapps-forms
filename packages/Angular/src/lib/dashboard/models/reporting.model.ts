...
export interface FormReportData {
   ...
   * pivots these onto `responses` (Complete only, so a partial's rows never reach the sheet); the
   * Insights use the complete-only subset `loadReport` derives from them.
   *
