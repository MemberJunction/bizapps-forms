/**
 * The defect: a signature drawn in section two is GONE after Back then Next.
 *
 ...
 * empty box reading "Draw your signature above." over an answer that is, in fact, stored.
 *
 * Every other control in `form-question.component.html` renders from `value()`. The signature pad
 * is the one that renders from itself, which is why it is the one that loses its contents.
 *
...
const drawing = (bytes = 'ink'): File =>
  new File([bytes], 'signature.png', { type: 'image/png' });

/**
 * Section 1: a name. Section 2: two signatures and a file upload.
 *
 * Two signature questions rather than one because "the wrong question's drawing" is a place a
 * repaint can land — OneQuestion mode renders the whole deck through one component instance, so
 * two consecutive Signature questions are served by the SAME pad.
 */
function twoSectionForm(): PublishedFormDefinition {
  const question = (id: string, type: 'ShortText' | 'Signature' | 'FileUpload', order: number) => ({
    id,
    type,
    prompt: id,
    isRequired: false,
  ...
  return {
    ...
    pages: [
      { id: 'p1', displayOrder: 1, questions: [question('q-name', 'ShortText', 1)] },
      {
        ...
        displayOrder: 2,
        questions: [
          question('q-sign', 'Signature', 1),
          question('q-countersign', 'Signature', 2),
...
/**
 * Draw a signature and let its upload finish — the exact sequence
 * `FormQuestionComponent.uploadFile` runs.
 */
function sign(store: FormUploadStore, questionId: string, ink = 'ink'): File {
...
describe('a signature drawn in section two survives Back then Next', () => {
  it('leaves the answer in place — this half already worked', () => {
    ...
    expect(runtime.valueFor('q-sign')).toBe('file-q-sign');
  ...
  it('leaves the DRAWING where a re-created pad can find it — this half is the bug', () => {
    ...
    // stored answer.
    expect(store.viewFor('q-sign').file).toBe(png);
  });
  ...
  it('shows a file upload as already uploaded — the contrast that isolates the defect', () => {
    ...
    // same navigation leaves it looking answered. Signature renders from the canvas instead.
    expect(store.viewFor('q-id-scan')).toMatchObject({ status: 'done', fileName: 'licence.pdf' });
  });
});

describe('one pad, two signature questions', () => {
  it('never offers one question the drawing made for another', () => {
    ...
    // screen — reading "Signed." over an unanswered question. Whatever the pad is handed for
    // this question has to be this question's.
    expect(store.viewFor('q-countersign').file).toBeNull();
    expect(store.viewFor('q-countersign').status).toBe('idle');
  });

  it('keeps the two drawings apart when both are signed', () => {
    const { store } = connectedStore();
    const first = sign(store, 'q-sign', 'first');
    const second = sign(store, 'q-countersign', 'second');

    expect(store.viewFor('q-sign').file).toBe(first);
    expect(store.viewFor('q-countersign').file).toBe(second);
  });
});
...
describe('the drawing tracks the answer through every outcome', () => {
  it('is still there after a failed upload, so a retry shows what they drew', () => {
    ...
    const token = store.begin('q-sign', png);
    store.fail(token, 'The upload did not go through. Please try again.');

    expect(runtime.valueFor('q-sign')).toBeUndefined();
    expect(store.viewFor('q-sign').file).toBe(png);
  });
  ...
  it('is gone once the respondent clears the pad', () => {
    ...
    expect(runtime.valueFor('q-sign')).toBeUndefined();
    expect(store.viewFor('q-sign').file).toBeNull();
  });

  it('is gone for a question that was never signed', () => {
    const { store } = connectedStore();
