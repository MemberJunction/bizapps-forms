/**
 * Starter template blueprints — the no-AI "easy setup" path (FORMS_BUILD_PLAN §7).
 *
 * Each entry is a ready-made {@link FormBlueprint} the builder can instantiate verbatim
 * (then the user tweaks). They go through the SAME deterministic Builder as the AI path, so
 * they're guaranteed to conform to the §5.3 taxonomy and the contract shapes — and they
 * double as known-good fixtures for the builder tests.
 *
 * The gallery's *identity* (key, name, description, icon) is NOT here: it lives in
 * `STARTER_TEMPLATE_CATALOG` in `@mj-biz-apps/forms-entities`, which forms-ng reads too. This
 * file supplies only the half that is server-side — the blueprint each key expands to — and
 * the `Record<StarterTemplateKey, …>` below makes that mapping total, so adding a key to the
 * catalogue fails THIS file's compile until its blueprint exists.
 */
import {
...
/** A gallery entry: the catalogue's display metadata plus the blueprint to instantiate. */
export interface StarterTemplate extends StarterTemplateInfo {
  blueprint: FormBlueprint;
}

const contactBlueprint: FormBlueprint = {
  name: 'Contact us',
...
const rsvpBlueprint: FormBlueprint = {
  name: 'Event RSVP',
...
const npsBlueprint: FormBlueprint = {
...
const leadCaptureBlueprint: FormBlueprint = {
  name: 'Get in touch',
...
const applicationBlueprint: FormBlueprint = {
...
/**
 * Blueprint per starter key. Total by construction: the `Record` type makes a key added to
 * `STARTER_TEMPLATE_CATALOG` a compile error here until it has a blueprint, which is the
 * whole point of splitting identity from payload.
 */
const BLUEPRINTS: Record<StarterTemplateKey, FormBlueprint> = {
  contact: contactBlueprint,
  rsvp: rsvpBlueprint,
  nps: npsBlueprint,
  'lead-capture': leadCaptureBlueprint,
  application: applicationBlueprint,
};

/** The full starter gallery, in catalogue order. */
export const STARTER_TEMPLATES: readonly StarterTemplate[] = STARTER_TEMPLATE_CATALOG.map(
  (info) => ({ ...info, blueprint: BLUEPRINTS[info.key] }),
);
...
export function getStarterTemplate(key: string): StarterTemplate | undefined {
  const info = starterTemplateInfo(key);
  return info ? { ...info, blueprint: BLUEPRINTS[info.key] } : undefined;
}
