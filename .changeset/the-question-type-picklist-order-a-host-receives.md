...
filters on exactly that list. The rename moved the value from alphabetical slot 20 to slot 5, so the
designer's question-type dropdown rendered `Doodle` where `Signature` used to sit, with 15 other
values off by one — permanently, on every host.
