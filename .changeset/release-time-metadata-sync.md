...
No migration ships with this change, and none of the three existing `Metadata_Sync` files is rewritten — they are applied on hosts, and migrations are append-only history. This changes the cadence going forward; the next one is the first consolidated release seed.
