...
Seven metadata migrations, in order: `V202608211700` (the three staged prompts, their templates and the extended Form Designer blueprint), `V202608211800` (`?`-marks the optional keys in four `OutputExample`s — MJ treats every unsuffixed key as required, which was burning all three retries on every generation), `V202608211900` (pins the four authoring prompts to Gemini 3.6 Flash), `V202608212000` (the chat action, prompt and template), `V202608212100` (the chat's `image` action), `V202608212200` (the `edit` and `open` actions, the structural operations they carry, and a sixth template param for the form list) and `V202608212300` (the operation vocabulary the prompt states: the corrected handle rule, the 25 question types, and `updateOption`). Apply ALL of them before deploying the code — the client resolves `Forms: Chat` by name, so the name is a contract, and code that emits `edit` or `open` against a prompt seeded only to `V202608212100` is talking to a prompt that was never told those operations exist.
